...
import { heroImg, lazyImg, PORTRAIT, SCENE } from '../lib/img.ts'
import { chapterOrFace, faceId, useHashScroll } from '../lib/useHashScroll.ts'
...
export function CouncilPage() {
  ...
  useHashScroll('council', chapterOrFace)
    ...
    <div className="council">
      ...
      <main id={MAIN_ID} tabIndex={-1}>
        <section className="room-hero">
          <img src={`${asset('council/got/small-council-guide.jpg')}?v=got`} alt="" className="room-hero-art" {...heroImg()} />
        ...
        {councilChapters.map((chapter, index) => (
          ...
          >
            <figure className="council-scroll-art">
              <img src={`${asset(chapter.image)}?v=got`} alt="" {...lazyImg(SCENE)} />
            </figure>
          ...
          <div className="council-place-grid">
            {councilChairs.map((chair) => (
              <article key={chair.id} id={chair.id} className="council-place">
                <img src={asset(chair.image)} alt={chair.name} {...lazyImg(SCENE)} />
            ...
            return (
              <div key={bench} className="council-side">
                ...
                <div className="council-face-grid">
                  {faces.map((face) => (
                    <article key={face.id} id={faceId(face.id)} className="council-face">
                      <img src={`${asset(face.image)}?v=got`} alt={face.name} {...lazyImg(PORTRAIT)} />
                      <div className="council-copy">
