...
import { heroImg, LANDSCAPE, lazyImg, PORTRAIT, WIDE } from '../lib/img.ts'
    ...
    <div className="north">
      ...
      <main id={MAIN_ID} tabIndex={-1}>
        <section className="north-hero">
          <img src={asset('north/got/hero.jpg')} alt="" className="north-hero-art" {...heroImg()} />
        ...
        <section className="north-scroll" id="why">
          <figure className="north-scroll-art">
            <img {...lazyImg(WIDE)} src={asset(whyTheWall.image)} alt="Leaf of the Children of the Forest holds the dragonglass that made the first White Walker" />
          </figure>
          <div className="north-scroll-copy">
            ...
            {whyTheWall.body.map((paragraph, index) => (
        ...
        <section className="north-chapter" id="places">
          ...
          <div className="north-grid">
            {northPlaces.map((place) => (
              ...
              >
                <img src={asset(place.image)} alt={place.name} {...lazyImg(LANDSCAPE)} />
          ...
          <div className="north-grid north-grid-clans">
            {northClans.map((clan) => (
              <article key={clan.id} id={clan.id} className="north-card north-card-clan">
                <img src={asset(clan.image)} alt={clan.name} {...lazyImg(LANDSCAPE)} />
                <div className="north-copy">
          ...
          <div className="north-figures">
            {northFigures.map((figure) => (
              <article key={figure.id} id={figure.id} className="north-figure">
                <img src={asset(figure.image)} alt={figure.name} {...lazyImg(PORTRAIT)} />
                <div className="north-copy">
          ...
          <div className="north-grid">
            {coldOnes.map((item) => (
              <article key={item.id} id={item.id} className="north-card north-card-cold">
                <img src={asset(item.image)} alt={item.name} {...lazyImg(LANDSCAPE)} />
                <div className="north-copy">
        ...
        <section className="north-chapter north-chapter-wall" id="wall">
          ...
          <article className="north-wall">
            <img src={asset(wallFromNorth.image)} alt="The Wall from the north" {...lazyImg(WIDE)} />
            <div className="north-copy">
              <p className="north-scroll-lede">{wallFromNorth.lede}</p>
              {wallFromNorth.body.map((paragraph, index) => (
                <p key={index}>{paragraph}</p>
              ))}
            ...
            <div className="north-wall-pair">
              <figure>
                <img {...lazyImg(LANDSCAPE)} src={asset(wallFromNorth.abandoned)} alt="Abandoned castles of the Night’s Watch" />
                <figcaption>The unmanned keeps. Names the Watch still recites.</figcaption>
              </figure>
              <figure>
                <img {...lazyImg(LANDSCAPE)} src={asset(wallFromNorth.gate)} alt="A gate through the Wall" />
